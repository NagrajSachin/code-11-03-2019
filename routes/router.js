...
router.route('/customer/:customerId/:pagenumber/:limit')
.get((req,res,next)=>{
    var pagenumber = req.params.pagenumber;
    var size = parseInt(req.params.limit);
    Customer.find({supervisorId : req.params.customerId}).skip(5 * (pagenumber -1)).limit(size)
    .then(data => {
        res.status(200).json(data);
    }).catch((err)=> next(err));
})
